...
impl FieldV {
    ...
    /// Change the modulus of this field element; newly added to change the underlying curve for the r1cs circuit
    pub fn update_modulus(&mut self, new_modulus: Arc<Integer>) {
        ...
        match full_element {
            FullFieldV::IntField(ref mut i) => {
                ...
                // *i = i.clone().mod_floor(new_modulus);
            }
            _ => {panic!("Not applicable to non-integer field elements");}
        }
    }
    /// Change the value of this field element; newly added to change the underlying curve for the r1cs circuit
...
impl FieldV {
    ...
    pub fn is_zero(&self) -> bool {
        match self.either_ref() {
            ...
            Err(FullFieldV::FBls12381(pf)) => bool::from(pf.is_zero_vartime()),
            Err(FullFieldV::FBn254(pf)) => bool::from(pf.is_zero_vartime()),
...
impl Into<Integer> for &FieldV {
    fn into(self) -> Integer {
        if self.is_full() { // same as self.tag() == FieldTag::FullField
            self.full_ref().into()
        } else {
            let i = self.inline().0;
            if i < 0 {
