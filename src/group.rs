...
use super::scalar::{Scalar as WrappedScalar};
// end my additions
...
type GFp = ModInt256<
    0x20CD9255FD615923u64,
    0xACAFC103CD968A25u64,
...
// my additions
pub type Affine = PointAffineExtended;
impl GroupElement {
    pub fn generator() -> Self {
        Self::BASE
    }
  pub fn from_affine(point: Affine) -> Self {
    Self::from_affine_extended(&point)
  }
...
impl CompressedGroupExt for CompressedGroup {
    ...
    fn unpack(&self) -> Result<Self::Group, ProofVerifyError> {
        if let Some(r) = GroupElement::decode(&self[..]) {
            Ok(r)
        } else {
            Err(ProofVerifyError::DecompressionError(self.clone()))
...
impl GroupElement {
    ...
    */
    ...
    pub fn decode(buf: &[u8]) -> Option<GroupElement> {
        let mut P = GroupElement::NEUTRAL;
    ...
    fn lookup_affine_extended(win: &[PointAffineExtended; 16], k: i8) -> PointAffineExtended {
        // Split k into its sign s (0xFFFFFFFF for negative) and
        // absolute value (f).
        let s = ((k as i32) >> 8) as u32;
        let f = ((k as u32) ^ s).wrapping_sub(s);
    ...
    pub fn set_mulgen(&mut self, n: &Scalar) {
        ...
        *self = Self::from_affine_extended(&Self::lookup_affine_extended(&PRECOMP_B, sd[12]));
        self.set_add_affine_extended(&Self::lookup_affine_extended(&PRECOMP_B65, sd[25]));
        self.set_add_affine_extended(&Self::lookup_affine_extended(&PRECOMP_B130, sd[38]));
        self.set_add_affine_extended(&Self::lookup_affine_extended(&PRECOMP_B195, sd[51]));

        // Process the digits in high-to-low order.
        for i in (0..12).rev() {
            self.set_xdouble(5);
            self.set_add_affine_extended(&Self::lookup_affine_extended(&PRECOMP_B, sd[i]));
            self.set_add_affine_extended(&Self::lookup_affine_extended(&PRECOMP_B65, sd[i + 13]));
            self.set_add_affine_extended(&Self::lookup_affine_extended(&PRECOMP_B130, sd[i + 26]));
            self.set_add_affine_extended(&Self::lookup_affine_extended(&PRECOMP_B195, sd[i + 39]));
        }
    }
        ...
        J::Item: Borrow<Self>,
    {
        ...
        let ss_booth: Vec<_> = scalars
            .into_iter()
            .map(|s| Self::recode_scalar_ext(s.borrow(), w))
            .collect();
...
impl AddAssign<GroupElement> for GroupElement {
    #[inline(always)]
    fn add_assign(&mut self, other: GroupElement) {
        self.set_add(&other);
    }
}
...
impl MulAssign<u64> for GroupElement {
    #[inline(always)]
...
impl SubAssign<GroupElement> for GroupElement {
    #[inline(always)]
    fn sub_assign(&mut self, other: GroupElement) {
        self.set_sub(&other);
    }
}
...
//  my additions
impl Mul<WrappedScalar> for GroupElement {
    type Output = GroupElement;

    #[inline(always)]
    fn mul(self, other: WrappedScalar) -> GroupElement {
        let mut r = self;
        r.set_mul(&other.0);
        r
    }
...
impl MulAssign<WrappedScalar> for GroupElement {
    #[inline(always)]
    fn mul_assign(&mut self, other: WrappedScalar) {
        self.set_mul(&other.0);
    }
}
...
impl Mul<GroupElement> for WrappedScalar {
    type Output = GroupElement;
...
impl VartimeMultiscalarMul for GroupElement {
    ...
    where
        ...
        Self: Clone,
    {
        GroupElement::xmuladd_vartime(scalars, points)
    }
}
...
mod tests {
    ...
    use super::{GFp, GroupElement, Scalar};
    ...
    */
    ...
    fn encode_decode() {
        for i in 0..KAT_DECODE_OK.len() {
            ...
            let Q = GroupElement::decode(&buf).unwrap();
            assert!(Q.encode()[..] == buf);
        }
        for i in 0..KAT_DECODE_BAD.len() {
            let buf = hex::decode(KAT_DECODE_BAD[i]).unwrap();
            assert!(GroupElement::decode(&buf).is_none());
        }
    }
    ...
    fn base_arith() {
        for i in 0..KAT_ADD.len() {
            ...
            let P1 = GroupElement::decode(&buf1).unwrap();
            let P2 = GroupElement::decode(&buf2).unwrap();
            let P3 = GroupElement::decode(&buf3).unwrap();
            let P4 = GroupElement::decode(&buf4).unwrap();
            let P5 = GroupElement::decode(&buf5).unwrap();
            let P6 = GroupElement::decode(&buf6).unwrap();
            ...
            assert!((R6 + GroupElement::NEUTRAL).encode()[..] == buf6);
        }
    }

    #[test]
    fn mulgen() {
        let sbuf = hex::decode("f17dbcbef04a157b7e470b6563940017e5de0bbf30042ef0a86e36f4b8600d14")
            .unwrap();
        let (s, ok) = Scalar::decode32(&sbuf);
        assert!(ok == 0xFFFFFFFF);
        let rbuf = hex::decode("d79870ba92af7240e4e7a9ca4a96a6411822382b043006c9046fa8d011f79ace")
            .unwrap();
        let R = GroupElement::decode(&rbuf).unwrap();
        let P = GroupElement::BASE * s;
        assert!(P.equals(R) == 0xFFFFFFFF);
        assert!(P.encode()[..] == rbuf);
        let Q = GroupElement::mulgen(&s);
    ...
    fn mul() {
        for i in 0..20 {
            ...
            let s3 = s1 * s2;
            let P1 = GroupElement::mulgen(&s1);
            let Q1 = s1 * GroupElement::BASE;
            assert!(P1.equals(Q1) == 0xFFFFFFFF);
            let P2 = GroupElement::mulgen(&s3);
            let Q2 = s2 * Q1;
            assert!(P2.equals(Q2) == 0xFFFFFFFF);
        }

        let mut T = GroupElement::BASE.xdouble(120);
        assert!(
            T.encode()[..]
                == hex::decode("9811a66139b0779ecdd85726b290a67346aedda26bc1f6fded6da44631fb2149")
                    .unwrap()
        );
        for _ in 0..1000 {
        ...
        assert!(
            T.encode()[..]
                == hex::decode("7c9d21debf079763b76125728bbe6f8d22947ad296a86a92819654ac1b9bd135")
                    .unwrap()
        );
    }
    ...
    fn map() {
        for i in 0..50 {
            ...
            let P = GroupElement::map_to_curve(&f);
            assert!(P.Z.iszero() == 0);
            let iZ = GFp::ONE / P.Z;
            let e = P.E * iZ;
            let u = P.U * iZ;
            ...
            assert!(
                e.square()
                    .equals(-u.xsquare(2) + GroupElement::Ap * u.square() + GFp::ONE)
                    == 0xFFFFFFFF
            );
        }
    }
    ...
    fn test_recode_scalar_ext(s: &Scalar, w: i32) {
        let sd = GroupElement::recode_scalar_ext(s, w);
        let num = GroupElement::num_recoded_digits(w);
    ...
    fn test_xmuladd(n: usize) {
        ...
        let mut points = Vec::<GroupElement>::with_capacity(n);
        let z = (n as u64) << 32;
        for i in 0..n {
            let y = z + ((i as u64) << 2);
            ...
            if i == 40 {
                points.push(GroupElement::NEUTRAL);
            } else {
                points.push(GroupElement::hash_to_curve(&vp));
            }
        }

        let P = GroupElement::xmuladd_vartime(&scalars, &points);
